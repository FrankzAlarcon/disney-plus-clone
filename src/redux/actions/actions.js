...
const SEARCHED_ALL = (query) => `https://api.themoviedb.org/3/search/multi?api_key=${import.meta.env.VITE_API_KEY}&language=en-US&query=${query}&page=1&include_adult=false`;
...
export const getSearchedMovies = (query) => (dispatch) => {
  try {
    console.log(query.split(' ').join('%20'));
